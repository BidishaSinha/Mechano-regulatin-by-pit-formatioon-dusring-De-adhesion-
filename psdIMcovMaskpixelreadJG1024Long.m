...
for ii=1:2048
	A=imread([dname,'/',listings(Nstart+ii-1).name])./conv; %320 is for covertin to h
	B=A;
...
if wannasave == 'y'
    ...
    % filename=['allpsd3test_' sprintf('%d',Nr) '_' sprintf('%d',Mr)  rem '.mat'];
    save(filename,'allpsd3','Nstart','Nend', '-v7.3');
    end
